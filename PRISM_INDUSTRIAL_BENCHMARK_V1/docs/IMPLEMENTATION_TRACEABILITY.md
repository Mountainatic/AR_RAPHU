...
| Zero external to K/C/W/A candidate spaces | Strict nested-OOF amendment | `strict_oof_selection.py`, `v211_k.py`, `v211_c.py`, `v211_w.py`, `v211_a.py` | `test_strict_oof_selection.py` | stage `selection` and `routing_status` fields | TESTED |
| Strict positive nested-OOF risk admission; no one-SE/min-gain/fold/CI/bootstrap authority | Strict nested-OOF amendment | `strict_oof_selection.py`, active v2.1.1 stage and Joint paths | `test_strict_oof_selection.py` | parent/child OOF risks, `incremental_gain`, `epsilon_num` | TESTED |
| Zero does not block downstream residual routing | Strict nested-OOF amendment | `apply_stage_increment`, v2.1.1 W/A routing | `test_zero_increment_does_not_block_a_later_stage` | stage identity reason and downstream result | TESTED |
