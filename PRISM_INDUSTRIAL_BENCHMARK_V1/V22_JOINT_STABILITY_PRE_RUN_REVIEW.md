...
Full regression status is `179 passed, 1 skipped`. The artifact reuse audit is PASS, including unchanged shared-data aggregate, K/C/W/A RESULT files, C/W OOF files, unchanged K/C/W/A estimator sources, and `test_accessed=false`, `ood_accessed=false`. The formal M5 implementation treats real-artifact legacy-anchor mismatch as a hard stop before evaluating the v2.2 predictive path. M7/M8 and test/OOD remain out of scope.
