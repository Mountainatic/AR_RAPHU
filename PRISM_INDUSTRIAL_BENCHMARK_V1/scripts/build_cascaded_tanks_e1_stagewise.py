...
def _freeze(data_path: Path, source_root: Path, output_root: Path) -> None:
    ...
    if not source_freeze_path.is_file():
        # The current Tanks runner writes its freeze at the run root.
        source_freeze_path = source_root / "DEVELOPMENT_FREEZE.json"
...
def _report(data_path: Path, source_root: Path, output_root: Path) -> None:
    ...
    if not source_prediction_path.is_file():
        source_prediction_path = source_root / "TEST_PREDICTIONS.parquet"
