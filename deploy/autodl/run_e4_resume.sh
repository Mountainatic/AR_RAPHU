...
"${UV_BIN}" run python tools/make_phase1_scheme_a_manifest.py \
  --scenario AR-S3 --track XAR --stage warmup --device cuda
"${UV_BIN}" run python tools/make_phase1_scheme_a_manifest.py \
  --scenario AR-S3 --track XAR --stage fork --device cuda
...
run_pool results/phase1/manifests/AR-S3_G2_XAR_warmup.json
run_pool results/phase1/manifests/AR-S3_G2_XAR_fork.json
"${UV_BIN}" run python tools/run_phase1_scheme_a.py select \
  --scenario AR-S3 --track XAR --device cuda
"${UV_BIN}" run python tools/run_phase1_scheme_a.py aggregate \
  --scenario AR-S3 --track XAR --device cuda
echo "E4 Scheme-A Track-XAR completed."
