...
"${UV_BIN}" run python tools/make_phase1_m7_manifest.py
"${UV_BIN}" run python STAGE1_DUAL_SOLVER_V20_bundle/tools/run_gpu_job_pool.py \
  --manifest results/phase1/manifests/AR-S1_G2_M7_candidates.json \
  --devices 0 \
...
"${UV_BIN}" run python tools/run_phase1_m7.py select --device cuda
"${UV_BIN}" run python tools/run_phase1_m7.py aggregate --device cuda
echo "M7 validation selection and post-freeze test aggregation completed."
