...
"${UV_BIN}" run python tools/make_phase1_m6_manifest.py
"${UV_BIN}" run python STAGE1_DUAL_SOLVER_V20_bundle/tools/run_gpu_job_pool.py \
  --manifest results/phase1/manifests/AR-S2_G2_M6_candidates.json \
  --devices 0 \
...
"${UV_BIN}" run python tools/run_phase1_m6.py select --device cuda
"${UV_BIN}" run python tools/run_phase1_m6.py aggregate --device cuda
echo "M6 validation selection and post-freeze test aggregation completed."
