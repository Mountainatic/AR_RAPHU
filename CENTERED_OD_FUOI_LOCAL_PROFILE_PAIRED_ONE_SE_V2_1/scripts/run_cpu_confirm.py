...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument("--n-jobs", "--profile-workers", dest="n_jobs", type=int, default=8)
    value.add_argument("--bootstrap-jobs", "--bootstrap-workers", dest="bootstrap_jobs", type=int, default=16)
...
def main() -> int:
    ...
    required = (args.shared, args.cpu_baselines, args.gpu_baselines, args.v1_results)
    if any(value is None for value in required):
        raise SystemExit("shared, cpu-baselines, gpu-baselines and v1-results are required on first run")
    ...
    invocation.write_text(json.dumps({
        ...
        "gpu_baselines": args.gpu_baselines, "v1_results": args.v1_results, "protocol": args.protocol,
